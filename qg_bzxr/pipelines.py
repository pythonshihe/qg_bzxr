...
class QgBzxrPipeline(object):
    def __init__(self):
        self.mysql_client = MysqlClient(host='****', port=3306, user='root', password='axy#mysql2019', db='shihe_data')
    ...
    def process_item(self, item, spider):
        ...
        if oname:
            try:
                self.mysql_client.insert('qg_bzxr', item)
                print("主体:%s" % item["oname"])
                ...
                if "Duplicate" in repr(e):
                    ...
                    # print('插入失败,错误：%s' % repr(e))
                    logging.error('插入失败,错误：%s' % repr(e))
        return item
